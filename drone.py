...
class Drone:
    ...
    def send_fixed_absolute_position(self, positionX, positionY, positionZ, velocity, heading, rotationalVelocity):
        """
        drone movement command
        ...
        data.positionZ = float(positionZ) - self.get_position_data()[3]
        data.velocity = float(velocity)
        data.heading = heading
        data.rotationalVelocity = rotationalVelocity

        c=sqrt(data.positionX**2 + data.positionY**2)
        temp=sqrt(c**2 + data.positionZ**2)
